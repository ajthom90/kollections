...
open class DelegatingMutableTable<R, C, V>(private val map: MutableMap<R, MutableMap<C, V>>): MutableTable<R, C, V> {
	...
	override fun transpose(): MutableTable<C, R, V> {
		return DelegatingMutableTable(columnMap())
	}

	private fun getColumnMap(rowKey: R): MutableMap<C, V> {
		val cm = map[rowKey]
		val out = if (cm == null) {
	...
	override fun asTable(): Table<R, C, V> {
		return DelegatingTable(map.toMap().mapValues { it.value.toMap() })
	}
}
